...
if __name__ == "__main__":
    ...
    with args.output as csvfile:
        fieldnames = ['sample_size', 'seq_length', 'fastARG_time', 'N_c_records_orig','N_c_records_fastARG', 'simulation_seed']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames, delimiter="\t")
        writer.writeheader()
        for params in sample_size_sims + length_sims:
            seed = seed_generator.randint(1,2^32-1)
            ...
            with NamedTemporaryFile("w+") as fa_in, \
                ...
                run_fastARG("../fastARG/fastARG", fa_in, fa_out)
                end_time = time.time()
